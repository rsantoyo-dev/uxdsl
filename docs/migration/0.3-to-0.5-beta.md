...
> checkout de `postcss-uxdsl` (versión de paquete `0.3.0`, sin publicar).
> No hay un release `0.5.0-beta.x` real todavía — este documento existe
> para que la migración se pueda validar y ajustar antes de que exista uno.
> Ver [FEAT-002](../features/FEAT-002-beta-migration-hardening.md) para el
> contexto completo (hallazgos, prioridades, estado de cada mejora).
...
- **Tamaños compuestos o "recetas" configurables** (por ejemplo, un `size`
  ...
  Solo `radius()`/`shadow()` son overridables hoy; ver MIG-05 en
  FEAT-002 para el resto de opciones evaluadas y no implementadas.
- **`includeTheme: false` sin pasar `theme`** (para que el componente no
  ...
  validen y los nombres de variable coincidan. Documentado como pendiente
  de decisión en MIG-02.
- **Migración automática de `radius(N)` cuando el manual override no es una
  llamada `radius()`/`shadow()` pura** (un literal, `calc()`, o una
  expresión responsive) — el codemod deliberadamente no toca esos casos;
  hay que decidirlos a mano (ver más abajo).
...
- Ningún panel vuelve a declarar `:root`, así que un loader de CSS Modules
  ...
  Next.js + `css-loader` instalando paquetes empaquetados sigue pendiente
  de MIG-07.
...
node packages/postcss-uxdsl/scripts/codemod-size-overrides.js src/**/*.uxdsl

# Aplica los cambios
node packages/postcss-uxdsl/scripts/codemod-size-overrides.js --write src/**/*.uxdsl
```

O, parado en `packages/postcss-uxdsl`: `npm run codemod:size-overrides -- [--write] <archivos>`.
...
si hay más de una declaración candidata en la misma regla, si el valor no
es una llamada `radius()`/`shadow()` pura (un literal, `calc()`, una
expresión responsive), o si la declaración tiene `!important`, el codemod
**no toca esa regla** y lo reporta como caso a revisar manualmente en la
salida (`SKIPPED`).
...
`test/codemod-size-overrides.test.js` — `npm test` en
`packages/postcss-uxdsl` corre los 103 casos, incluidos estos.
