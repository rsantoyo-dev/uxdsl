...
| Campo | Evidencia |
| --- | --- |
| SHA base / entrega / PR | Base `049b5c0` (2026-09-22, HEAD de `main` y de esta rama al crearla). Entrega: commit inicial en `feat/mig-b7-01-placeholder-tone` (hash fijado por un commit de fixup inmediatamente posterior a este, mismo patrón usado para MIG-B6-21 en FEAT-008). PR: pendiente de abrir; no puede auto-mergearse (el clasificador de auto-modo bloquea `gh pr merge` con "Merge Without Review") — requiere que el usuario lo ejecute. |
