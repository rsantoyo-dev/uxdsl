...
- [x] `release.js` aplica los presupuestos y verifica los dist-tags después de
      ...
      seguimiento")**
- [x] La URL de Google Fonts está codificada. **(cerrado una vez entregada
      MIG-B6-21, que era la condición que esta misma ficha ponía. `index.ts`
      importa y usa `googleFontsImportUrls` de `./fonts` — el codificador
      compartido que entregó MIG-B6-29 fase 4 — en vez de construir una
      segunda codificación aquí, que es justo lo que el paso 5 prohibía.
      `test/fonts.test.js` cubre `;` y comas de ejes
      (`Inter:wght@400;500;600;700`, `Roboto Flex:opsz,wght@8..144,100..1000`,
      `Nunito Sans:ital,wght@0,400;1,400`) y el ejemplo literal de esta ficha,
      `Open Sans:wght@400;700` → `family=Open+Sans:wght@400;700`. El comentario
      de cabecera de `index.ts`, que describía un prototipo de cinco líneas,
      se reescribió con la superficie real.)**
