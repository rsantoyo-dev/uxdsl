...
| MIG-B6-29 | [El JSON base es la única fuente de defaults](MIG-B6-29-json-base-unica-fuente.md) | B — Tema base y salida correcta | P0 · L | 15 | Fases 1-3/4 implementadas/verificadas en `feat/feat-008-beta6-plan` (fase 1: `a078c51`, corrección de revisión en `fe23608`; fase 2: `4917193`; fase 3: ver ficha) — fase 1 movió el JSON + extrajo 7 familias + migró el playground; fase 2 construyó el gate de contraste `checkThemeContrast`; fase 3 corrigió 16 colores de `theme/base.json` (y 10+15 de los temas `green`/`slate` del playground) en OKLCH, mínimo cambio — `report.passed` sigue en `false` a propósito: quedan 3 hallazgos reales de arquitectura/motor (no de color) documentados con recomendación de seguimiento, y el helper de fuentes (fase 4) sigue pendiente — ver "Límites y seguimiento" de la ficha; integración a `main` pendiente |
