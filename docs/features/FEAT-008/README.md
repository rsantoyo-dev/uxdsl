...
| MIG-B6-29 | [El JSON base es la única fuente de defaults](MIG-B6-29-json-base-unica-fuente.md) | B — Tema base y salida correcta | P0 · L | 15 | Fases 1-2/4 implementadas/verificadas en `feat/feat-008-beta6-plan` (fase 1: `a078c51`, corrección de revisión en `fe23608`; fase 2: ver ficha) — fase 1 movió el JSON + extrajo 7 familias + migró el playground; fase 2 construyó el gate de contraste `checkThemeContrast` (real, verificado, reproduce los ratios de la ficha) — `report.passed` sigue en `false` contra el tema real porque la corrección de color (fase 3) y el helper de fuentes (fase 4) siguen pendientes, por pedido del dueño dado el tamaño/impacto excepcionales — ver "Límites y seguimiento" de la ficha; integración a `main` pendiente |
...
| MIG-B6-28 | [Higiene de paquetes y npm](MIG-B6-28-higiene-npm.md) | G — Release | P0 · S | 18, 21, 27, 29 | Implementada/verificada parcialmente en `feat/feat-008-beta6-plan` (`a103344`, corrección de revisión en `c899127`) sin esperar 21/27/29 — la parte de Google Fonts en `index.ts` queda deliberadamente sin hacer (la propia ficha la secuencia después de 21); resto del alcance (files, manifiesto, release.js) completo — ver "Límites y seguimiento" de la ficha; integración a `main` pendiente |
