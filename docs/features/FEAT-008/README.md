...
| MIG-B6-12 | [Gate de release beta.6 (re-alcanzado)](MIG-B6-12-gate-beta6.md) | G — Release | P0 · M | todas las demás | Implementada/verificada en `feat/feat-008-beta6-plan` (ver ficha) — `npm run verify:beta6`: 26/26 comprobaciones sobre los cinco tarballs; release record en `docs/releases/0.5.0-beta.6.md` con `automated`/`browser`/`external`/`postpublish` separados. Verificado que el gate **puede fallar** revirtiendo MIG-B6-15 y MIG-B6-22 — esa comprobación destapó que el propio gate no hacía `await`, y de paso encontró un bug real: `theme --contrast \| jq` truncaba el JSON. **Press Craftor y los dist-tags siguen pendientes y son del dueño**; integración a `main` pendiente |
